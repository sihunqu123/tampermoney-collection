...
(function() {
  ...
  const loadCustomStyle = () => {
    ...
    const customCSS = `
      ...
      .tb-skin div:nth-child(1) > dl.tm-img-prop > dt {
        display: none !important;
      }
      .itemContainer {
        max-height: 900px;
  ...
  const patchA = () => {
    // find all li
    let allItems = Array.from(document.querySelectorAll("#J_isku div:nth-child(1) > dl:nth-child(1) > dd:nth-child(2) > ul > li"));
    // this work for tmall
    if(allItems.length === 0) {
      allItems = Array.from(document.querySelectorAll(".tb-skin div:nth-child(1) > dl.tm-img-prop > dd:nth-child(2) > ul > li"));
    }
    if(allItems.length > 0) {
